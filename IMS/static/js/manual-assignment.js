// Manual Assignment Page Functionality
...
function setupAllotFunctionality() {
    ...
    confirmAllotBtn.addEventListener('click', function() {
        ...
        assignStaffToSlot(
            staffId, 
            staffName, 
            window.currentSlot.date, 
            window.currentSlot.session, 
            window.currentSlot.hallNo
        );
...
function fetchAvailableStaff(date, session, hallCategory, hallDept) {
    ...
    staffRows.forEach(row => {
        ...
        const staffDepartment = cells[2].textContent.trim();
        const staffDeptCategory = cells[3].textContent.trim();
        ...
        if (staffDeptCategory === hallCategory &&
            staffDepartment !== hallDept &&
            !isBusy) {
                    ...
                    window.staffAssignmentsMap[staffId] = data.assignments;
                            ...
                            if (data.assignments.length > 0) {
                                ...
                                data.assignments.forEach(assignment => {
                                    assignmentsHTML += `
                                        <tr>
                                            <td class="px-4 py-2">${assignment.date}</td>
                                            <td class="px-4 py-2">${assignment.session}</td>
                                            <td class="px-4 py-2">${assignment.hall_no}</td>
                                            <td class="px-4 py-2">${assignment.dept_name}</td>
                                            <td class="px-4 py-2">${assignment.dept_category}</td>
                                        </tr>
                                    `;
                                });
